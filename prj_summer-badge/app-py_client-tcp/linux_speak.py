...
def play_phrase(user_info:dict):
    ...
    subprocess.run("espeak -v it -s 150 -a 80 '{0}'".format(phrase),shell=True)
...
def get_phrase(user_info:dict) -> list:
    ...
    if rand(0,2) == 0:
        if name == "Matteo" and surname == "Palazzolo":
            ...
            return ["àh ruffì, accendi il radar"]
    ...
    if rand(0,2) == 0:
        if ("Chitarra" in activities) and rand(0,4):
            ...
            return ["Dis is not a Jojo referens"]
    ...
    if classroom != "Docente" or (name == "Marco" and surname == "Bracco"):
        return [
            ...
            f"è spaunato un {name} selvatico",
            f"Ciao {name}, benvenuto nella criù",
        ...
        return ["Buongiorno", "Entri pure","Buona Giornata"]

    
############# CODE #############
if __name__ == "__main__":
    play_phrase({'name':"Matteo","surname":"Palazzolo","classroom":"3IS","birth_date":"01-01-2020","activities":["Chitarra","Fumetto - manga"]})